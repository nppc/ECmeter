...
***************************************************************************************************/
 ...
 void i2c_Delay(void){
	 ...
	 //i++;
	 //i++;
	 // very short delay. Can be extended if I2C bus will have issues.
 }
...
//
static bool i2cByteOut(uint8_t b)
{
	uint8_t i;
	bool ack;
	...
	{
		if (b & 0x80)
			SDA_PIN = 1;	// set data line to 1
		else
			SDA_PIN = 0; 	// set data line to 0
		SCL_PIN = 1; // clock high (slave latches data)
		i2c_Delay();
		SCL_PIN = 0; // clock low
		b <<= 1;
	} // for i
	// read ack bit
	SDA_PIN = 1; // set data line for reading
	SCL_PIN = 1; // clock line high
	i2c_Delay();
	ack = SDA_PIN;
	SCL_PIN = 0; // clock low
	i2c_Delay();
	SDA_PIN = 0; // data low
  return !ack; //(ack == 0) ? 1:0; // a low ACK bit means success
}
*/
...
{
	//bool rc;
	SDA_PIN = 0; // data line low first
	i2c_Delay();
	SCL_PIN = 0; // then clock line low is a START signal
	//I2C_Start();
	addr <<= 1;
	if (bRead)
		addr++; // set read bit
	//rc = i2cByteOut(addr); // send the slave address and R/W bit
	I2C_Write(addr); // send the slave address and R/W bit
	return 1;
}
